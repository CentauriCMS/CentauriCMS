                                        ...
                                        <div class="row be-users-row m-0">
                                            @foreach($beuser->getRoles() as $role)
                                                @if(isset(centauriconfig("be_roles")[$role->name]))
                                                    ...
                                                    <button class="{{ $overwriteClass }}" data-uid="{{ $role->uid }}" onclick="Centauri.fn.__editRecord(event, this, 'uid', 'BackendRole', 'editInEditorComponent')">
                                                        {{ $role->name }}
                                                    </button>
                                                @else
                                                    <button class="btn waves-effect waves-light p-2" data-uid="{{ $role->uid }}">
                                                        {{ $role->name }}
                                                    </button>
                                                @endif
                                            @endforeach
